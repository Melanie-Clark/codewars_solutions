...
Then, if any people have the same number of countries visited, sort them by the person's name ascending.
...
-- number of countries visited by each person
SELECT
    p.name,
...
GROUP BY
    ...
    p.name
ORDER BY
  COUNT(DISTINCT v.country_id) DESC,
  p.name ASC